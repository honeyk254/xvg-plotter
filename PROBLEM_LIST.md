...
| **v1.3.0** | **Phase 3 shipped (2026-09-13); localization declined.** **C21** (full session restore: checked files, active file + dataset, hidden series, per-series colors, style/analysis state, zoom, annotations), **C11** (every dataset of a multi-dataset file listed and overlayable; dataset-qualified legend labels and CSV export), **C15** (text annotations: ✎ Text toolbar toggle, click-to-place, draggable, kept in exports/prints, View ▸ Clear annotations), **C18** (grid view / small multiples: View ▸ Grid view of checked files, Ctrl+G, 24-panel cap) and **C24** (derived quantities: Normalize by first value/max, Subtract baseline, dashed least-squares Fit line with the equation in the legend). Localization (C35) was evaluated and declined for now. Remaining are conscious v1 boundaries only: C01 signing (no budget — checksums + RELEASE.md), C02 auto-update (user-triggered check only), C12 sibling formats (non-goal), C35 localization (declined). |
...
**C35 · No localization.** English-only UI (backlog, PRD §9); terminology like "replica averaging" is a hurdle for non-native speakers. Freq. 1 (P16) · Severity Low · Status: backlog — a localization pass was implemented in v1.3.0 and then **removed by decision of the maintainer** (English-only app). The codebase keeps standard Qt `tr()` wrappers, so a translation catalog can be added later without touching the UI code again.
