...
**C20 · Dock-heavy layout crowds small screens.** On 1366×768 (and at 200 % scaling) the three docks leave a sliver of canvas; no compact/zen mode. Freq. 2 (P15, P19) · Severity Medium · Status: **largely covered in v1.0.2** — Style moved into a collapsible tab above the canvas, removing the worst offender; the left file/series docks still take space on 768p. Residual: compact/focus handling for the left docks.

**C21 · No session save/restore.** Selected files, styles, zoom survive until the app closes; yesterday's comparison is gone (only folders/geometry persist, SPEC §9). Freq. 2 (P03, P04) · Severity Low · Status: open — v1.0.2 fixed the file-switch zoom bug (view cache keys on plotted data identity) and restores both folder panes, but checked files, styles and zoom are still not restored across restarts. → Persist last plot state like last folder.

**C22 · Single-window/single-instance prevents side-by-side work.** PRD §7 *requires* single-instance forwarding, but the effect is that two projects/folders can never be compared on two monitors, and a second launch hijacks the current selection. Freq. 4 (P09, P10, P26, P27) · Severity Medium · Status: **largely covered in v1.0.2** — dual folder panes plus curve pins overlay two systems in one window (with a "mixed X axes" warning), which answers the P10/P26 workflow; P09's literal two-monitor/two-window case remains. Residual: opt-in "open in new window".
