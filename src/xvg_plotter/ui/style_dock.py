...
class StyleDock(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.chk_grid.setToolTip(self.tr("Draw a light grid on the plot"))
        self.chk_logx = QCheckBox()
        self.chk_logx.setToolTip(self.tr("Logarithmic X axis"))
        self.chk_logy = QCheckBox()
        self.chk_logy.setToolTip(self.tr("Logarithmic Y axis"))
        self.cmb_legend = QComboBox()
        self.cmb_legend.addItems(LEGEND_LOCS)
        self.cmb_legend.setToolTip(self.tr(
            "Legend position — 'outside right' keeps it off the data"))
        self.cmb_palette = QComboBox()
        self.cmb_palette.addItems(list(PALETTES))
        self.cmb_palette.setToolTip(self.tr(
        ...
        self.cmb_line.setToolTip(self.tr("Line / marker style for every plotted series"))
        self.spin_width = QDoubleSpinBox()
        self.spin_width.setRange(0.5, 6.0)
        self.spin_width.setSingleStep(0.5)
        self.spin_width.setValue(1.5)
        self.spin_width.setToolTip(self.tr("Line width"))
        ...
        self.cmb_unit.setToolTip(self.tr("Rescale the time axis (auto picks the largest "
                                     "unit; only applied to genuine time axes)"))
        ...
        self.spin_figw.setSpecialValueText(self.tr("auto"))
        self.spin_figw.setValue(float(settings.get("view/fig_w", 0.0) or 0.0))
        self.spin_figw.setToolTip(self.tr("Figure width in inches (auto = fill the window)"))
        ...
        self.spin_figh.setSpecialValueText(self.tr("auto"))
        ...
        form.addRow(self.tr("Legend"), self.cmb_legend)
        form.addRow(self.tr("Colors"), self.cmb_palette)
        form.addRow(self.tr("Line"), self.cmb_line)
        form.addRow(self.tr("Width"), self.spin_width)
        sizrow = QHBoxLayout()
        sizrow.addWidget(self.spin_figw)
        sizrow.addWidget(self.spin_figh)
        form.addRow(self.tr("Fig size (in)"), sizrow)
        form.addRow(self.tr("Font"), self.cmb_font)
        form.addRow(self.tr("Title"), self.ed_title)
        form.addRow(self.tr("X label"), self.ed_xlabel)
        form.addRow(self.tr("Y label"), self.ed_ylabel)
        form.addRow(self.tr("X unit"), self.cmb_unit)
